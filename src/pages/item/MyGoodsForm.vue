  ...
  export default {
    ...
    methods: {
      ...
      submit() {
        ...
        const skus = this.skus
          .filter(s => s.enable)
          .map(({ id,price, stock, enable, images, indexes, ...rest }) => {
    ...
    watch: {
      ...
      "goods.brandId":{
        ...
        handler(val){
          if (val) {
            this.$http.get("item/brand/" + val).then(({data}) => {
              this.selectedBrand = data;
              // 将specs属性中的品牌属性内容改为选中的品牌
              this.specs.map(s => {
                if (s.name === "品牌") {
                  s.v = data.name;
                }
